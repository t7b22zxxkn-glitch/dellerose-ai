...
export async function getLatestPersistedWorkflow(): Promise<WorkflowQueryResult> {
  ...
  try {
    ...
    const { data: briefRow, error: briefError } = await supabase
      .from("briefs")
      .select(
        "workflow_id, source_transcript, core_message, intent, target_audience, key_points, emotional_tone"
      )
      .eq("user_id", userId)
    ...
    if (briefError) {
      return {
        snapshot: null,
        notice: "Kunne ikke hente latest workflow fra Supabase.",
      }
    }
    ...
    const { data: postRows, error: postsError } = await supabase
      ...
      .eq("user_id", userId)
      .eq("workflow_id", parsedBriefRow.data.workflow_id)
    ...
    const brief = contentBriefSchema.parse({
      coreMessage: parsedBriefRow.data.core_message,
      intent: parsedBriefRow.data.intent,
      targetAudience: parsedBriefRow.data.target_audience,
      keyPoints: parsedBriefRow.data.key_points,
      emotionalTone: parsedBriefRow.data.emotional_tone,
    })

    const drafts = parsedPostRows.data.map((post) =>
    ...
    const postPlans = parsedPostRows.data.map((post) =>
    ...
    return {
      snapshot: {
        workflowId: parsedBriefRow.data.workflow_id,
        transcript: parsedBriefRow.data.source_transcript,
        brief,
        drafts,
        postPlans,
        chatLog: [
